...
std::unique_ptr<WindowChannel> WindowChannel::RegisterWithRegistrar(
    ...
    int64_t window_id) {
  auto window_registrar =
      flutter::PluginRegistrarManager::GetInstance()
          ->GetRegistrar<flutter::PluginRegistrarWindows>(registrar);
  auto channel =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          window_registrar->messenger(),
...
WindowChannel::WindowChannel(
  ...
  channel_->SetMethodCallHandler(
      [this](const flutter::MethodCall<Argument>& call, auto result) {
        if (!handler_) {
          std::cout << "WindowChannel::SetMethodCallHandler: handler_ is null"
                    << std::endl;
          return;
        }
        auto* args = std::get_if<flutter::EncodableMap>(call.arguments());
        auto target_window_id =
            args->at(flutter::EncodableValue("targetWindowId")).LongValue();
        auto arguments = args->at(flutter::EncodableValue("arguments"));
        handler_(window_id_, target_window_id, call.method_name(), &arguments,
                 std::move(result));
      });
}
...
void WindowChannel::InvokeMethod(
  ...
  channel_->InvokeMethod(
      ...
      std::make_unique<flutter::EncodableValue>(flutter::EncodableMap{
          {flutter::EncodableValue("fromWindowId"),
           flutter::EncodableValue(from_window_id)},
