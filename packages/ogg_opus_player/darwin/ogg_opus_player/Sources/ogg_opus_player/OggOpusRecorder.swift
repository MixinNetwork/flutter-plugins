...
final class OggOpusRecorder {
  ...
  private var speechTranscriber: OggOpusSpeechTranscriber?
  ...
  func cancel(for reason: CancelledReason, userInfo: [String: Any]? = nil) {
    processingQueue.async {
      ...
      self.close(finishTranscription: false)
...
extension OggOpusRecorder {
  private func startRecording() throws {
    ...
    try self.startSpeechTranscriptionIfNeeded()
  ...
  private func stopRecording() {
    processingQueue.async {
      ...
      self.close(finishTranscription: true)
  ...
  private func close(finishTranscription: Bool) {
  ...
  fileprivate func process(size: Int, render: (inout AudioBufferList) -> OSStatus) -> OSStatus {
    ...
    processingQueue.async { [weak self] in
      ...
      if #available(iOS 10.0, macOS 10.15, *) {
        do {
          try self.speechTranscriber?.appendPCMData(pcmData, sampleRate: self.sampleRate)
        } catch {
          self.speechTranscriber?.cancel()
          self.speechTranscriber = nil
          DispatchQueue.main.async {
            self.delegate?.oggOpusRecorder(self, didFailTranscriptionWithError: error)
          }
        }
      }
    }
  ...
  private func startSpeechTranscriptionIfNeeded() throws {
    guard let transcriptionConfiguration else {
      return
    }
    guard #available(iOS 10.0, macOS 10.15, *) else {
      throw Error.speechRecognitionUnavailable
    }

    let transcriber = try OggOpusSpeechTranscriber(
      localeIdentifier: transcriptionConfiguration.localeIdentifier,
      addsPunctuation: transcriptionConfiguration.addsPunctuation,
      shouldReportPartialResults: true,
      resultHandler: { [weak self] transcription in
        guard let self else {
          return
        }
        DispatchQueue.main.async {
          self.delegate?.oggOpusRecorder(self, didReceiveTranscription: transcription)
        }
      },
